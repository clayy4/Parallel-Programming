...
### verification.py  
Python-скрипт для автоматической верификации результата умножения матриц.  
Сравнивает результат работы C++ программы с эталонным вычислением через NumPy и выводит результат проверки.

### benchmark.py  
Скрипт для построения графика зависимости времени выполнения умножения матриц от их размера.  
Использует результаты экспериментов и сохраняет график в PNG-файл.

### matrix1.txt / matrix2.txt  
Входные файлы с исходными матрицами.

### result.txt  
Файл с результатом перемножения матриц, сформированный программой на C++.
